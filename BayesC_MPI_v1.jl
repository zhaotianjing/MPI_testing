...
function IIBLMM()
    ...
    for iter = 1:5
      ...
      # sample intercept
      rhs_mu  = sum_y + dot(J,ya) #scaler all  ! not same   sum_y not same!!
...
# function sampleVariance(α, nLoci, dfEffectVar, scaleVar)
#     return (dot(α,α) + dfEffectVar*scaleVar)/rand(Chisq(nLoci+dfEffectVar))
# end
